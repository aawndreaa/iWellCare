...
<?php $__env->startSection('title', 'Doctor Availability - iWellCare'); ?>
<?php $__env->startSection('page-title', 'Doctor Availability'); ?>
<?php $__env->startSection('page-subtitle', 'Manage doctor availability for appointments and consultations'); ?>
...
<div class="doctor-availability-content">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
            <div class="flex items-start justify-between mb-4">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        <h3 class="text-lg font-semibold text-gray-900">Dr. <?php echo e($doctor->user->first_name ?? ''); ?> <?php echo e($doctor->user->last_name ?? ''); ?></h3>
                        <p class="text-sm text-gray-500"><?php echo e($doctor->specialization ?? 'General Medicine'); ?></p>
                    </div>
                </div>
                <?php
                ...
                ?>
                <span class="px-3 py-1 rounded-full text-xs font-medium <?php echo e($status['is_available'] ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'); ?>">
            ...
            <div class="space-y-2 mb-4">
                <div class="flex items-center text-sm">
                    ...
                    <span class="ml-2 <?php echo e($status['is_available'] ? 'text-green-600' : 'text-red-600'); ?>">
                        <?php echo e($status['message']); ?>

                    </span>
                </div>

                <?php if($latestSetting && !$status['is_available']): ?>
                    <?php if($latestSetting->unavailable_until): ?>
                    <div class="flex items-center text-sm">
                        <span class="font-medium text-gray-700 w-20">Until:</span>
                        <span class="ml-2 text-gray-600"><?php echo e($latestSetting->unavailable_until->format('M d, Y H:i')); ?></span>
                    </div>
                    <?php endif; ?>
                    <?php if($latestSetting->notes): ?>
                    <div class="flex items-center text-sm">
                        <span class="font-medium text-gray-700 w-20">Notes:</span>
                        <span class="ml-2 text-gray-600"><?php echo e(Str::limit($latestSetting->notes, 40)); ?></span>
                    </div>
                    <?php endif; ?>
                <?php endif; ?>

                <?php if($latestSetting): ?>
                <div class="text-xs text-gray-500 pt-3 border-t border-gray-100">
                    <div class="flex justify-between">
                        <span><strong>Set by:</strong> <?php echo e($latestSetting->setBy->first_name ?? ''); ?> <?php echo e($latestSetting->setBy->last_name ?? 'System'); ?></span>
                        <span><strong>Updated:</strong> <?php echo e($latestSetting->updated_at->diffForHumans()); ?></span>
                    </div>
                </div>
                <?php endif; ?>
            </div>
            ...
            <div class="flex justify-center">
                <button onclick="setUnavailable(<?php echo e($doctor->id); ?>)" class="w-full max-w-xs bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 transform hover:scale-105">
    ...
    <!-- Recent Activity -->
    <?php if($availabilitySettings->count() > 0): ?>
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg font-semibold text-gray-900">Recent Availability Changes</h3>
            <span class="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full"><?php echo e($availabilitySettings->count()); ?> changes</span>
        </div>
        <div class="space-y-3">
            <?php $__currentLoopData = $availabilitySettings->take(8); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $setting): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                <div class="flex items-center space-x-3">
                    ...
                    <div class="flex-1">
                        <p class="font-medium text-gray-900">Dr. <?php echo e($setting->doctor->user->first_name ?? ''); ?> <?php echo e($setting->doctor->user->last_name ?? ''); ?></p>
                        <p class="text-sm text-gray-600">
                            Set to <span class="font-medium <?php echo e($setting->is_available ? 'text-green-600' : 'text-red-600'); ?>"><?php echo e($setting->is_available ? 'Available' : 'Unavailable'); ?></span>
                            by <?php echo e($setting->setBy->first_name ?? ''); ?> <?php echo e($setting->setBy->last_name ?? 'System'); ?>

                        </p>
                    </div>
                </div>
                <div class="text-right">
                    <p class="text-sm font-medium text-gray-900"><?php echo e($setting->created_at->format('M d, Y')); ?></p>
                    <p class="text-xs text-gray-500"><?php echo e($setting->created_at->format('H:i')); ?></p>
                    <?php if($setting->status !== 'available'): ?>
                    <p class="text-xs text-gray-400 mt-1"><?php echo e(ucfirst($setting->status)); ?></p>
                    <?php endif; ?>
                </div>
            </div>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
        <?php if($availabilitySettings->count() > 8): ?>
        <div class="mt-4 text-center">
            <p class="text-sm text-gray-500">Showing 8 of <?php echo e($availabilitySettings->count()); ?> changes</p>
        </div>
        <?php endif; ?>
    </div>
    <?php endif; ?>
...
<?php echo $__env->make('layouts.staff', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\iWellCare\resources\views/staff/doctor-availability/index.blade.php ENDPATH**/ ?>