            ...
            <div class="bg-white rounded-xl p-6 border border-gray-200 shadow-lg hover:shadow-xl transition-shadow duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-gray-900 mb-2">{{ $invoices->total() }}</p>
            ...
            <div class="bg-white rounded-xl p-6 border border-gray-200 shadow-lg hover:shadow-xl transition-shadow duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-gray-900 mb-2">{{ $invoices->filter(fn($b) => $b->status === 'paid')->count() }}</p>
            ...
            <div class="bg-white rounded-xl p-6 border border-gray-200 shadow-lg hover:shadow-xl transition-shadow duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-gray-900 mb-2">{{ $invoices->filter(fn($b) => $b->status === 'unpaid')->count() }}</p>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-xl">
            <div class="p-6 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('admin.invoice.create') }}" class="inline-flex items-center justify-center px-5 py-2.5 bg-gradient-to-r from-green-600 to-green-700 text-white font-semibold rounded-lg hover:from-green-700 hover:to-green-800 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <div class="font-semibold text-indigo-600">{{ $invoice->invoice_no ?? 'INV-' . str_pad($invoice->id, 6, '0', STR_PAD_LEFT) }}</div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 bg-gradient-to-br from-blue-500 to-blue-600 rounded-full flex items-center justify-center text-white font-bold shadow-md">
                                            {{ strtoupper(substr($invoice->patient->user->first_name ?? $invoice->patient->first_name ?? '-', 0, 1)) }}
                                        </div>
                                        <div class="min-w-0">
                                            <div class="font-medium text-gray-900">
                                                {{ $invoice->patient->user->first_name ?? $invoice->patient->first_name ?? '-' }} {{ $invoice->patient->user->last_name ?? $invoice->patient->last_name ?? '' }}
                                            </div>
                                            <div class="text-xs text-gray-500 truncate">{{ $invoice->patient->user->email ?? $invoice->patient->email ?? '' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($invoice->appointment)
                                        <div class="flex items-center gap-2">
                                            ...
                                            <div>
                                                <div class="font-medium text-gray-900">{{ $invoice->appointment->appointment_date->format('M d, Y') }}</div>
                                                <div class="text-xs text-gray-500">{{ $invoice->appointment->appointment_time->format('h:i A') }}</div>
                                            </div>
                                        </div>
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="font-bold text-lg text-gray-900">₱{{ number_format($invoice->grand_total ?? $invoice->total_amount ?? $invoice->amount ?? 0, 2) }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($invoice->payment_date)
                                        <div class="flex items-center gap-2">
                                            ...
                                            <div>
                                                <div class="font-medium text-gray-900">{{ \Carbon\Carbon::parse($invoice->payment_date)->format('M d, Y') }}</div>
                                                <div class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($invoice->payment_date)->format('h:i A') }}</div>
                                            </div>
                                        </div>
                                    @else
                                        <span class="text-gray-400">-</span>
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <a href="{{ route('admin.invoice.create') }}" class="mt-2 inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 transition-colors duration-200">
                                            <i class="fas fa-plus mr-2"></i> Create Invoice
                                        </a>
                                    </div>
                                </td>
