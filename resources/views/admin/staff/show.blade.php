...
@section('page-subtitle', 'Team member details')
            ...
            <div class="bg-gradient-to-r from-blue-600 to-purple-600 px-6 py-8 text-white">
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <h3 class="text-2xl font-bold">{{ $staff->first_name }} {{ $staff->last_name }}</h3>
                    ...
                    <div>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-900">{{ $staff->first_name }} {{ $staff->last_name }}</p>
                            </div>
                            <div>
                    ...
                    <div>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-900">{{ $staff->street_address }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-500">City</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $staff->city }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-500">State/Province</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $staff->state_province }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-500">Postal Code</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $staff->postal_code }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-8 pt-8 border-t border-gray-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-900">{{ $staff->created_at->format('M d, Y \a\t g:i A') }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Last Updated</label>
                            <p class="mt-1 text-sm text-gray-900">{{ $staff->updated_at->format('M d, Y \a\t g:i A') }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-8 pt-8 border-t border-gray-200">
                    <div class="flex items-center justify-between">
                        <div class="flex space-x-4">
                            <a href="{{ route('admin.staff.edit', $staff) }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                <i class="fas fa-edit mr-2"></i>
                                Edit Member
                            </a>
                            <form method="POST" action="{{ route('admin.staff.toggle-status', $staff) }}" class="inline">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white {{ $staff->is_active ? 'bg-red-600 hover:bg-red-700 focus:ring-red-500' : 'bg-green-600 hover:bg-green-700 focus:ring-green-500' }} focus:outline-none focus:ring-2 focus:ring-offset-2">
                                    <i class="fas {{ $staff->is_active ? 'fa-user-times' : 'fa-user-check' }} mr-2"></i>
                                    {{ $staff->is_active ? 'Deactivate' : 'Activate' }}
                                </button>
                            </form>
                        </div>
                        <form method="POST" action="{{ route('admin.staff.destroy', $staff) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this team member? This action cannot be undone.')">
                            @csrf
                            @method('DELETE')
