...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
    <div class="card p-4 flex flex-col items-center bg-gradient-to-br from-blue-100 to-blue-50 shadow-md">
        ...
        <div class="text-2xl font-bold text-blue-700">{{ $appointments->total() }}</div>
    ...
    <div class="card p-4 flex flex-col items-center bg-gradient-to-br from-yellow-100 to-yellow-50 shadow-md">
        ...
        <div class="text-2xl font-bold text-yellow-700">{{ $appointments->where('status', 'pending')->count() }}</div>
    ...
    <div class="card p-4 flex flex-col items-center bg-gradient-to-br from-green-100 to-green-50 shadow-md">
        ...
        <div class="text-2xl font-bold text-green-700">{{ $appointments->where('status', 'confirmed')->count() }}</div>
    ...
    <div class="card p-4 flex flex-col items-center bg-gradient-to-br from-gray-100 to-gray-50 shadow-md">
        ...
        <div class="text-2xl font-bold text-gray-700">{{ $appointments->where('status', 'completed')->count() }}</div>
...
<div class="card p-6 mb-6">
    ...
    <form method="GET" action="" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('staff.appointments.index') }}" class="btn btn-secondary flex items-center gap-2">
                        ...
                        <td class="py-4 px-6">
                            <div class="flex items-center gap-3">
                                <span class="inline-flex items-center justify-center w-10 h-10 bg-green-100 rounded-full text-green-600 font-bold text-lg flex-shrink-0">
                                    {{ strtoupper(substr($appointment->doctor->user->first_name ?? '-', 0, 1)) }}
                                </span>
                                <div class="min-w-0 flex-1">
                                    <div class="font-medium truncate">Dr. {{ $appointment->doctor->user->first_name ?? '-' }} {{ $appointment->doctor->user->last_name ?? '' }}</div>
